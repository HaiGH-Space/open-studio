...
import { getExporter, exportPrompt, AGENT_EXPORTERS } from "../src/lib/export"
...
describe("Agent-Specific Export Formatters", () => {
  ...
  function createSampleConfig(
    ...
    return {
      ...
      layer3AuthoritativeConstraints: {
        ...
        viewport: "responsive",
        strictHardRules: [
          "No third-party component libraries without permission",
        ],
      },
      layer5BrandContract: {
      ...
      layer9BriefAndClarification: {
        userObjective:
          "Build a high-converting pricing page with monthly/annual toggle.",
        featureRequirements: [
  ...
  describe("Claude Code Exporter (claude-code-exporter.ts)", () => {
    ...
    it("produces primary clipboard text optimized for Claude 3.7 Sonnet reasoning mode with XML delimiters", () => {
      ...
      expect(exportPkg.primaryClipboardText).toContain(
    ...
    it("generates a downloadable CLAUDE.md file with markdown formatting", () => {
      ...
      const claudeMd = exportPkg.downloadableFiles.find(
        (f) => f.filename === "CLAUDE.md"
      )
      ...
      expect(claudeMd?.content).toContain(
        "Always prefer semantic HTML5 elements"
      )
    ...
    it("includes downloadable prompt.xml with application/xml mimeType", () => {
      ...
      const promptXml = exportPkg.downloadableFiles.find(
        (f) => f.filename === "prompt.xml"
      )
  ...
  describe("Cursor Exporter (cursor-exporter.ts)", () => {
    ...
    it("splits System Rules vs. User Task into primary and secondary clipboard targets", () => {
      ...
      expect(exportPkg.primaryClipboardText).toContain("<brand-contract")
      expect(exportPkg.primaryClipboardText).not.toContain(
        "<brief-and-clarification"
      )
      ...
      expect(exportPkg.secondaryClipboardText).toContain(
        "Build a high-converting pricing page"
      )
      expect(exportPkg.secondaryClipboardText).toContain(
        "Starter, Pro, Enterprise"
      )
      expect(exportPkg.secondaryClipboardText).not.toContain(
        "<security-guardrails"
      )
    })
    ...
    it("generates a downloadable .cursorrules file with plain text mimeType", () => {
      ...
      const cursorrules = exportPkg.downloadableFiles.find(
        (f) => f.filename === ".cursorrules"
      )
  ...
  describe("Generic LLM Exporter (generic-llm-exporter.ts)", () => {
    ...
    it("generates downloadable prompt.md and prompt.xml files", () => {
      ...
      const promptMd = exportPkg.downloadableFiles.find(
        (f) => f.filename === "prompt.md"
      )
  ...
  describe("Exporter Registry & Helper Utilities", () => {
    ...
    it("throws a descriptive error when querying an unknown agent target", () => {
      expect(() =>
        getExporter("unknown-agent" as unknown as AgentTarget)
    ...
    it("exports directly via exportPrompt helper", () => {
      ...
      expect(
        claudeExport.downloadableFiles.some((f) => f.filename === "CLAUDE.md")
      ).toBe(true)

      const cursorExport = exportPrompt("cursor", result, config)
      expect(
        cursorExport.downloadableFiles.some(
          (f) => f.filename === ".cursorrules"
        )
      ).toBe(true)
      ...
      expect(
        genericExport.downloadableFiles.some((f) => f.filename === "prompt.md")
      ).toBe(true)
    })
  ...
  describe("Edge Cases & Defensive Behaviors", () => {
    it("handles empty user brief gracefully across all exporters", () => {
      ...
      // Claude Code
      const claudePkg = claudeCodeExporter.formatExport(
        result,
        emptyBriefConfig
      )
      ...
      // Generic LLM
      const genericPkg = genericLlmExporter.formatExport(
        result,
        emptyBriefConfig
      )
    ...
    it("handles special characters and XML/markdown syntax in user input", () => {
      const specialCharsConfig = createSampleConfig({
        layer9BriefAndClarification: {
          userObjective:
            "Build a <tag> & 'quote' \"table\" with price < $50 and > $10.",
    ...
    it("formats hard rules and persistent directives when present", () => {
      ...
      const claudeMd = claudePkg.downloadableFiles.find(
        (f) => f.filename === "CLAUDE.md"
      )?.content
      ...
      const cursorrules = cursorPkg.downloadableFiles.find(
        (f) => f.filename === ".cursorrules"
      )?.content
      ...
      expect(genericPkg.primaryClipboardText).toContain(
        "Must compile with zero warnings"
      )
      expect(genericPkg.primaryClipboardText).toContain(
        "Follow BEM class naming"
