...
export function LayerTokenStackedBar({
  ...
  const effectiveTotal =
    typeof totalTokens === "number" ? totalTokens : sumTokens
  ...
  return (
    ...
    >
      ...
      <div className="flex h-2 w-full overflow-hidden rounded-full border border-border/40 bg-muted/40 p-[1px]">
      ...
      {showLegend && activeSegments.length > 0 && (
        ...
        >
          {activeSegments.map((seg) => (
            ...
            >
              <span
                className={cn("size-2 shrink-0 rounded-full", seg.color.bg)}
              />
              <span className="max-w-36 truncate font-medium text-foreground/80">
